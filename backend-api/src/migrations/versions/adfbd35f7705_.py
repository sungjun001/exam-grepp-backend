...
Revision ID: adfbd35f7705
Revises: 7dff5d9f16b3
Create Date: 2024-06-27 21:24:34.589538
...
revision: str = 'adfbd35f7705'
down_revision: Union[str, None] = '7dff5d9f16b3'
...
def upgrade() -> None:
    ...
    op.drop_table('tier')
    op.drop_index('ix_exam_schedule_created_by_user_id', table_name='exam_schedule')
    op.drop_index('ix_exam_schedule_is_deleted', table_name='exam_schedule')
    ...
    op.drop_table('post')
    op.drop_index('ix_user_reservation_exam_schedule_id', table_name='user_reservation')
    op.drop_index('ix_user_reservation_user_id', table_name='user_reservation')
...
def downgrade() -> None:
    ...
    sa.Column('exam_schedule_id', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('status', postgresql.ENUM('RESERVED', 'CONFIRMED', 'CANCELLED', 'DELETED', name='reservationstatus'), autoincrement=False, nullable=False),
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['exam_schedule_id'], ['exam_schedule.id'], name='user_reservation_exam_schedule_id_fkey'),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], name='user_reservation_user_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='user_reservation_pkey'),
    sa.UniqueConstraint('user_id', 'exam_schedule_id', name='unique_user_exam_schedule')
    )
    op.create_index('ix_user_reservation_user_id', 'user_reservation', ['user_id'], unique=False)
    op.create_index('ix_user_reservation_exam_schedule_id', 'user_reservation', ['exam_schedule_id'], unique=False)
    ...
    sa.Column('reserve_count', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('confirm_count', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True),
    sa.Column('deleted_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True),
    sa.Column('is_deleted', sa.BOOLEAN(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['created_by_user_id'], ['user.id'], name='exam_schedule_created_by_user_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='exam_schedule_pkey'),
    sa.UniqueConstraint('uuid', name='exam_schedule_uuid_key')
    )
    op.create_index('ix_exam_schedule_is_deleted', 'exam_schedule', ['is_deleted'], unique=False)
    op.create_index('ix_exam_schedule_created_by_user_id', 'exam_schedule', ['created_by_user_id'], unique=False)
